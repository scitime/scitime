...
if __name__ == '__main__':
    ...
    parser.add_argument('--algo', required=False,
                        ...
                        choices=SUPPORTED_ALGOS, help='algo to train data on')

    parser.add_argument('--generate_data', required=False, default=False,
                        action='store_true',
                        help='''do you want to generate
                        & write data in a dedicated csv?''')

    parser.add_argument('--fit', required=False,
                        help='''do you want to fit the model?
                        If so indicate the csv name''')

    parser.add_argument('--save', required=False, default=False,
                        action='store_true',
                        help='''(only used for model fit) do you want to save
    ...
    if args.fit is None:
        if args.meta_algo is not None:
            raise ValueError('''No need to specify the meta algo
            if you only want to generate data''')
        if args.save:
            raise ValueError('''No need to specify the
            --save flag if you only want to generate data''')
